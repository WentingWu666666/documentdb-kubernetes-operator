				...
				Extensions: []cnpgv1.ExtensionConfiguration{
					{
						...
						ImageVolumeSource: corev1.ImageVolumeSource{
							Reference: "ghcr.io/documentdb/documentdb:0.116.0",
						},
					},
				},
			...
			Plugins: []cnpgv1.PluginConfiguration{
				{
					...
					Parameters: map[string]string{
						"gatewayImage":               "ghcr.io/documentdb/gateway:0.116.0",
...
var _ = Describe("SyncCnpgCluster", func() {
	...
	It("detects extension image changes", func() {
		current := baseCluster("test-cluster", namespace)
		desired := current.DeepCopy()
		desired.Spec.PostgresConfiguration.Extensions[0].ImageVolumeSource.Reference = "ghcr.io/documentdb/documentdb:0.116.0"
		...
		Expect(updated.Spec.PostgresConfiguration.Extensions[0].ImageVolumeSource.Reference).To(Equal("ghcr.io/documentdb/documentdb:0.116.0"))
	})

	It("detects gateway image changes", func() {
		current := baseCluster("test-cluster", namespace)
		desired := current.DeepCopy()
		desired.Spec.Plugins[0].Parameters["gatewayImage"] = "ghcr.io/documentdb/gateway:0.116.0"
		...
		updated := &cnpgv1.Cluster{}
		Expect(c.Get(context.Background(), types.NamespacedName{Name: "test-cluster", Namespace: namespace}, updated)).To(Succeed())
		Expect(updated.Spec.Plugins[0].Parameters["gatewayImage"]).To(Equal("ghcr.io/documentdb/gateway:0.116.0"))
	})
